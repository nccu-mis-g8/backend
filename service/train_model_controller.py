...
def chat():
    ...
    try:
        ...
        # 隨機決定是否生成兩個回應
        generate_two_responses = random.random() < 0.5  # 50% 機率生成兩個回答
        num_return_sequences = 2 if generate_two_responses else 1

        # 生成回應
        generated_responses = generate_text(prompt, max_length=70, num_return_sequences=num_return_sequences)

        # 返回多個回應供用戶選擇
        if num_return_sequences == 2:
            response_data = {
                "response_1": generated_responses[0],
                "response_2": generated_responses[1],
                "message": "Please choose the better response."
            }
        else:
            response_data = {"response": generated_responses[0]}
